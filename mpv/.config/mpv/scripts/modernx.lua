...
local osc_styles = {
  ...
  description = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H000000&\\fs"
    .. user_opts.description_font_size
    .. "\\q2\\fn"
    .. user_opts.font
    .. "}",
}
...
function process_vid_stats(success, result, error)
  ...
  if false then
    state.localDescriptionClick = state.localDescriptionClick
      :gsub("Views:", icons.emoticon.view)
      :gsub("Comments:", icons.emoticon.comment)
      :gsub("Likes:", icons.emoticon.like) -- replace with icons
  end

  if not state.ytdescription or #state.ytdescription < 5 then
    local vf_fps = mp.get_property_number("estimated-vf-fps")
    if vf_fps then
      state.videoDescription = mp.get_property("width")
        .. "x"
...
function check_description()
  if not user_opts.show_description then
    return
  end
...
function window_controls()
  ...
  if user_opts.window_title then
    ...
    lo.geometry = geo
    lo.style =
      string.format("%s{\\clip(0,%f,%f,%f)}", osc_styles.window_title, geo.y - geo.h, geo.x + geo.w, geo.y + geo.h)
...
layouts["original"] = function()
  ...
  if
    (user_opts.window_top_bar == "yes" or not state.border or not state.title_bar or state.fullscreen) and top_titlebar
  then
  ...
  if (state.localDescription ~= nil or state.is_URL) and user_opts.show_description then
    geo =
      { x = 25, y = refY - 117 + (user_opts.seekbar_between_timers and 25 or 0), an = 1, w = osc_geo.w - 50, h = 19 }
    lo = add_layout("description")
    lo.geometry = geo

    lo.style =
      string.format("%s{\\clip(0,%f,%f,%f)}", osc_styles.description, geo.y - geo.h, geo.x + geo.w, geo.y + geo.h)
  ...
  lo.geometry = {
    ...
    y = refY - 84,
    an = 9,
    w = 35 + (state.tc_ms and 30 or 0) + (show_remhours and 25 or 0),
    h = 20,
  }
  lo.style = osc_styles.time
  ...
  if user_opts.show_chapter_title then
    ...
    lo.geometry =
      { x = 65 + (state.tc_ms and 25 or 0) + (show_hours and 16 or 0), y = refY - 84, an = 7, w = 30, h = 20 }
    lo.style = osc_styles.time
    ...
    lo.geometry = {
      ...
      y = refY - 84,
      an = 7,
      w = osc_geo.w - 200 - ((show_hours or state.tc_ms) and 60 or 0),
      h = 20,
    }
  ...
  if info_button then
    ...
    lo.geometry = {
      x = osc_geo.w - 172 + (loop_button and 0 or 45) + (ontop_button and 0 or 45),
      y = refY - 40,
      an = 5,
      w = 24,
      h = 24,
  ...
  if screenshot_button then
    ...
    lo.geometry = {
      x = osc_geo.w - 217 + (loop_button and 0 or 45) + (ontop_button and 0 or 45) + (info_button and 0 or 45),
      y = refY - 40,
      an = 5,
      w = 24,
      h = 24,
...
layouts["reduced"] = function()
  ...
  lo.geometry = geo
  lo.style =
    string.format("%s{\\clip(0,%f,%f,%f)}", osc_styles.title, geo.y - geo.h, geo.x + osc_geo.w - 170, geo.y + geo.h)
  lo.alpha[3] = 0
...
local function osc_init()
  ...
  ne.tooltipF = function()
    ...
    if track_id ~= 0 then
      ...
      local prop = mp.get_property("current-tracks/audio/title") or mp.get_property("current-tracks/audio/lang")
  ...
  ne.eventresponder["shift+mbtn_right_down"] = function()
    mp.commandv("script-binding", "loadaudiotracks/ask_for_audio_track")
  end
  ...
  ne.content = function()
    return state.downloading and icons.download_initiated or icons.download
  end
  ne.visible = (
    osc_param.playresx
    >= 1100
  ...
  ne.tooltip_style = osc_styles.tooltip
  ne.tooltipF = function()
    return state.downloading and (texts.downloading .. "...")
      or (texts.download .. " (" .. state.file_size_normalized .. ")")
  end
  ne.eventresponder["mbtn_left_up"] = function()
    if not state.videoCantBeDownloaded then
      ...
      elseif state.downloading then
        ...
        local mpv_ytdl = (user_opts.ytdl_format and user_opts.ytdl_format ~= "") and user_opts.ytdl_format
          or mp.get_property("file-local-options/ytdl-format")
          or mp.get_property("ytdl-format")
          or ""
  ...
  ne.visible = (
    osc_param.playresx
    >= 800 - outeroffset - (user_opts.loop_button and 0 or 100) - (user_opts.ontop_button and 0 or 100)
  )
  ne.tooltip_style = osc_styles.tooltip
  ...
  if user_opts.volume_control then
    ...
    ne.slider.tooltipF = function(pos)
      return (get_track("audio") > 0) and set_volume(pos) or ""
    end
...
function adjust_subtitles(visible)
  if
    visible
    and user_opts.raise_subtitles
    and state.osc_visible == true
    and (state.fullscreen == false or user_opts.show_fullscreen)
  then
    local _, h = mp.get_osd_size()
    if h > 0 then
      local subpos = math.floor(
        (osc_param.playresy - (user_opts.raise_subtitle_amount - (user_opts.seekbar_between_timers and 25 or 0)))
          / osc_param.playresy
          * 100
      )
      if subpos < 0 then
  ...
  if not (state.anitype == nil) then
    ...
    if now < state.anistart + (user_opts.fade_duration / 1000) then
      if state.anitype == "in" then --fade in
        osc_visible(true)
        state.animation = scale_value(state.anistart, (state.anistart + (user_opts.fade_duration / 1000)), 255, 0, now)
      elseif state.anitype == "out" then --fade out
        state.animation = scale_value(state.anistart, (state.anistart + (user_opts.fade_duration / 1000)), 0, 255, now)
      end
    else
  ...
  elseif source == "mouse_move" then
    ...
    then
      if user_opts.bottom_hover then -- if enabled, only show osc if mouse is hovering at the bottom of the screen (where the UI elements are)
        ...
        if
          mouseY > osc_param.playresy - (user_opts.bottom_hover_zone or 200)
          or (user_opts.window_top_bar == "yes" or not state.border or not state.title_bar or state.fullscreen)
...
function tick()
  ...
  elseif (state.fullscreen and user_opts.show_fullscreen) or (not state.fullscreen and user_opts.show_windowed) then
    -- render the OSC
    render()
  else
    -- Flush OSD
  ...
  if state.anitype ~= nil then
    ...
    if
      not state.idle
      and (not state.anistart or mp.get_time() < 1 + state.anistart + user_opts.fade_duration / 1000)
    then
...
if user_opts.key_bindings then
  ...
  mp.add_key_binding(nil, "show_osc", function()
    show_osc()
  end)
end
