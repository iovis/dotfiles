...
alias geminit="r gem install awesome_print pry pry-doc ruby-lsp ruby-lsp-rails solargraph rubocop:1.66.1" # or whatever the bundle says (can conflict with `standard`)
...
function libupdate() {
  ...
  curl -Lo nvim "https://github.com/neovim/neovim-releases/releases/download/v0.11.4/nvim-linux-x86_64.appimage"
...
v() {
  if [[ "$PWD" == "$HOME/co/manage" || "$PWD" == "$HOME/co/mki_cli" ]]; then
    if [[ $# -gt 0 ]]; then
      BUNDLE_GEMFILE=$HOME/co/manage/Gemfile.ruby33.rails4 r nvim $@
    elif [[ -f "Session.vim" ]]; then
      BUNDLE_GEMFILE=$HOME/co/manage/Gemfile.ruby33.rails4 r nvim -S Session.vim
    else
      BUNDLE_GEMFILE=$HOME/co/manage/Gemfile.ruby33.rails4 r nvim
    fi
  else
...
lspsetup() {
  if [[ "$PWD" == "$HOME/co/manage" ]]; then
    r gem update awesome_print pry pry-doc ruby-lsp ruby-lsp-rails solargraph

    # if this fails do `rm -rf $HOME/.bundle/cache/`
    r bundle install --gemfile $HOME/co/manage/Gemfile.ruby33.rails4 --no-prune --no-cache
