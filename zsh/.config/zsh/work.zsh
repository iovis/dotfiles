...
function libupdate() {
  ...
  curl -Lo muxi.tar.xz "https://github.com/iovis/muxi/releases/download/v3.3.0/muxi-x86_64-unknown-linux-gnu.tar.xz"
  tar xf muxi.tar.xz
  install muxi*/muxi -D -t "$LOCAL_BIN"
  muxi --version

  echo "\n[$(date '+%Y-%m-%d %H:%M')] Installing neovim"
  # Pinned because of glibc version
  ...
  echo "\n[$(date '+%Y-%m-%d %H:%M')] Installing stylua"
  # Pinned because of glibc version
  curl -Lo stylua.zip "https://github.com/JohnnyMorganz/StyLua/releases/download/v2.0.2/stylua-linux-x86_64.zip"
  unzip stylua.zip
  install stylua -D -t "$LOCAL_BIN"
  stylua --version
