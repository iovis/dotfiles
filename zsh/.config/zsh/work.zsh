export BUNDLE_CACHE_PATH=/dev/null
...
alias jira="mki jira"
alias psv="ps auxf | nvim '+nmap q <cmd>qa!<cr>' '+nmap K <cmd>Tux sudo kill <cword><cr>' -"
...
v() {
  if [[ "$PWD" == "$HOME/co/manage" ]]; then
    if [[ $# -gt 0 ]]; then
      BUNDLE_GEMFILE=$HOME/co/manage/Gemfile.ruby33.rails5 chruby-exec ruby-3.3 -- nvim $@
    elif [[ -f "Session.vim" ]]; then
      BUNDLE_GEMFILE=$HOME/co/manage/Gemfile.ruby33.rails5 chruby-exec ruby-3.3 -- nvim -S Session.vim
    else
      BUNDLE_GEMFILE=$HOME/co/manage/Gemfile.ruby33.rails5 chruby-exec ruby-3.3 -- nvim
    fi
