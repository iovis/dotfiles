...
source.complete = function(self, _, callback)
  ...
  if not self.cache[bufnr] then
    Job
      :new({
        -- Uses `gh` executable to request the list from the remote repository.
        "gh",
        "pr",
        "list",
        "--limit",
        ...
        on_exit = function(job)
          ...
          for _, gh_item in ipairs(parsed) do
            ...
            -- TODO: is there a way to add the title to the description too?
            table.insert(items, {
...
require("cmp").register_source("gh_source", source.new())
